...
function allEqual(array: string[]) {
...
export function useCrossChainNetwork(
  ...
  const remoteUser = useMemo(() => {
    ...
    return layerZeroChainId
      ...
      : userAddress
  }, [chainId, userAddress, omniEscrowMap])
  ...
  } = useVotingEscrowLocksQuery(remoteUser)

  const votingEscrowLocks = useMemo(
    () => votingEscrowResponse?.votingEscrowLocks[0],
    [votingEscrowResponse]
  )

  const getNetworkSyncState = useCallback(
    (omniEscrowLock?: OmniEscrowLock | null, mainnetEscrowLock?: VotingEscrowLock) => {
  ...
  const calculateVeBAlBalance = useCallback(() => {
